...
const editWallet = new MenuTemplate(async ctx => {
  var reply
...
const enterAddress = new TelegrafStatelessQuestion("adr", async ctx => {
  ...
  if (dbWallet.address === ctx.session.wallet.address && !user.linked){
  ...
  else if (dbWallet.address === ctx.session.wallet.address && user.linked){

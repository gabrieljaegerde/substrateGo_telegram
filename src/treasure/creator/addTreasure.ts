...
const uploadQr = new TelegrafStatelessQuestion("qr", async (ctx: any) => {
    botParams.db.read()
    ...
    else if (ctx.session.qr && !ctx.session.qr.location.latitude || !ctx.session.qr.location.longitude) {

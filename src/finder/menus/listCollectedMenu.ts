...
const listCollected = new MenuTemplate(async (ctx: CustomContext) => {
  ...
  const userCollectedRewards: Array<IReward> = await Reward.find({ finder: ctx.chat.id, collected: true })
...
listCollected.chooseIntoSubmenu(
  ...
  showCollectedItem,
  {
    ...
    buttonText: async (ctx, key) => {
      ...
      if (key === "")
        return
      return session.userCollectedRewards.find((reward: IReward) => reward._id.equals(key)).name
    },

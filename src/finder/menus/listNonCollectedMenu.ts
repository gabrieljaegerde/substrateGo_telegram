...
export const listNonCollected = new MenuTemplate<CustomContext>(async (ctx) => {
  ...
  if (!session.userNonCollectedRewards || session.userNonCollectedRewards.length === 0)
    return `You don't have any treasures waiting to be collected at the moment.`;
...
export const listNonCollectedMiddleware = new MenuMiddleware('lnco/', listNonCollected);
